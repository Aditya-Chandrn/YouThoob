...
<?php
          while ($row = mysqli_fetch_assoc($result)) {
            ...
            <?php
            // Check if the video has been watched by the logged-in user
            $watched = false;
            $watchedQuery = "SELECT * FROM video WHERE email='$email' AND video_id='" . $row['video_id'] . "'";
            $watchedResult = mysqli_query($conn, $watchedQuery);
            if ($watchedResult->num_rows) {
                $watched = true;
            }
            
            ?>
        <div style="display: inline-block;">
        <a href="<?php echo $videoUrl; ?>" class="input13">
            <video width="320" height="240" controls autoplay loop>
                <source src="<?php echo '../uploaded_vid/' . $row['name']; ?>">
            </video>
            <div class="input17">
        ...
        <button data-video-id="<?php echo $row['video_id']; ?>">Delete</button>
            ...
            <?php
            // Display a message or mark the video as watched if it has been watched by the user
            if ($watched) {
                echo "<p>Watched</p>";
                // You can also add any additional logic or styling for watched videos
            }
        }
    }
}
