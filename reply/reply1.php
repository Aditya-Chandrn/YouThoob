...
if(isset($_POST['reply1'])){
    if(isset($_SESSION['email']) &&isset($_SESSION['id'])&& isset($_POST['cid'])) {
        $vid=$_SESSION['id'] ;
        $email = $_SESSION['email'];
        $cid = $_POST['cid'];
        $sql = "SELECT comment_id FROM comments WHERE comment_id='$cid'";
        ...
        $query = "INSERT INTO reply (text, email, comment_id, username, user_image, date, name, type, size) VALUES ('$comment',  '$email', '$cid', '$usname','$img1', '$date', '$filename', '$filetype', ' $filesize')";
        $stmt = mysqli_prepare($conn, $query);
        mysqli_stmt_execute($stmt);
        mysqli_stmt_close($stmt);
        // Redirect back to the video page
        $cid = $_POST['cid'];
        header("Location: ../comments/comments.php?id=".$vid."&cid=".$cid);
        exit();
    }
}
?>